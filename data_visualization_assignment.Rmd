...
dt %>%
  select(Liver) %>%
  ggplot(mapping = aes(x = Liver)) +
  geom_histogram(breaks = seq(0, 100, 2))

dt %>%
  select(Ovary) %>%
  ggplot(mapping = aes(x = Ovary)) +
  geom_histogram(breaks = seq(0, 100, 2))
 
# answer here
# The shape of the distribution is skewed to the left. This tells me that most genes are highly expressed. This is not specific to the liver.
```
...
ggplot(data = mitochondrial, aes(x = Liver + 1, y = `Heart - Left Ventricle` + 1, color = is_mitochondrial)) +
  geom_point(size = 1, alpha = 0.2) +
  geom_abline(slope = 1, lty = "dashed", color = "red") +
  scale_x_log10() +
  scale_y_log10() +
...
dt_long %>%
  ggplot(mapping = aes(x = TPM)) +
  geom_histogram(breaks = seq(0, 100, 2)) +
  facet_grid(Tissue ~ is_mitochondrial)
```
